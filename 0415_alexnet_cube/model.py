...
def inference(images):
    ...
    with tf.variable_scope('Alexnet') as scope:
        ...
        with tf.variable_scope('conv1') as scope:
            conv = tf.layers.conv2d(images, filters=64, kernel_size=[11, 11], strides=[4, 4],
                                    padding='same', data_format=format_data, kernel_regularizer=regularizer)
            conv1 = tf.nn.relu(conv)
            # tf.summary.image(scope.name, conv1)
        ...
        with tf.variable_scope('conv2') as scope:
            conv = tf.layers.conv2d(pool1, filters=192, kernel_size=[5, 5], strides=[1, 1],
                                    padding='same', data_format='channels_last', kernel_regularizer=regularizer)
...
def losses(logits, labels):
    with tf.variable_scope('loss') as scope:
        loss_l2 = tf.losses.get_regularization_losses()
        loss1 = 0.0
        for p in loss_l2:
            loss1 = tf.add(loss1, p)
        x = tf.reduce_sum(tf.multiply(logits, labels), axis=1)
        loss = tf.add(180.0 / math.pi * tf.acos(x), loss1)
        # loss = 180.0 / math.pi * tf.losses.cosine_distance(labels=labels, predictions=logits, axis=1)
...
def trainning(loss, learning_rate):
    with tf.name_scope('optimizer'):
        ...
        for i, (g, v) in enumerate(grads):
            if g is not None:
                grads[i] = (tf.clip_by_value(g, -6.0, 6.0), v)
